(function () {
    ...
    describe('MVC Models', function () {
        ...
        it('Call action method [HomeController.GetStudents] to get a array of \'Students\'', function (done) {
            ...
            xhr.done(function (result) {
                ...
                assert(_.isArray(students)
                    && students.length > 0
                    && _.all(students, function (el) { return _.has(el, 'FirstMidName'); }), 'Should receive a array of instances of $dpLib.ContosoUniversity.Models.Student');
                assert(_.all(students, function (el) { return el.FirstMidName === saved_settings_data.FirstMidName; }), 'All instances of $dpLib.ContosoUniversity.Models.Student should have the same FirstMidName (Alexandre)');
                done();
            });
            xhr.fail(function (xhr, status, message) {
                throw new Error(message);
        ...
        it('Call action method [HomeController.GetDepartment] to get the courses of \'SCIENCE\' Department', function (done) {
            ...
            xhr.done(function (result) {
                ...
                assert(department.Name === saved_settings_data.departmentName, 'Should receive a instance of $dpLib.ContosoUniversity.Models.Department');
                assert(_.isArray(department.Courses)
                    && department.Courses.length > 0
                    && _.all(department.Courses, function (el) { return el.CourseID > 0; }), 'Should receive courses of SCIENCE department (instances of $dpLib.ContosoUniversity.Models.Course)');
                done();
            });
            xhr.fail(function (xhr, status, message) {
                throw new Error(message);
    ...
    describe('WebApi Models', function () {
        ...
        it('Call action method [InstructorController.Put] to Add \'Courses\' to a instance of \'Instructor\'', function (done) {
            ...
            instructor_in.HireDate = new Date();
        ...
        it('action method OVERLOADING => [InstructorController.Put(instructor)] OR [InstructorController.Put(id, instructor)]', function (done) {
            ...
            var action = $dpUrlSet.$apiInstructor.Put.
                ...
                $action1;
            assert(typeof action.$Params().id === 'number', 'the method action should have a numeric parameter \'id\'');
            assert(typeof action.$Params().instructor === 'object' && typeof action.$Params().instructor.PersonID === 'number', 'the method action should have a instance of Instructor as parameter');
            //-- settings
            var settings = action.$AjaxSettings();
...
//# sourceMappingURL=ts_unit_test.js.map